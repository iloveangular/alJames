    ...
    <section class="packages first" v-if="username">
      <div class="container">
        <div class="row">
          <div class="col-md-12">
            <div class="title">
              <h2><span>Thanks for your Order, here are your details :</span></h2></div>
          </div>
        </div>
        <div class="row" id="complete">
          <div class="col-xs-12" v-for="order in orders">
            <div class="box-center" v-if="order._id == $route.params.orderID">
              <div class="testiram">
                <div class="row">
                  <div class="col-xs-12">
                    ...
                    <h5 v-if="order.stripeToken">Stripe Token: <span style="color: orange;">{{order.stripeToken}}</span>
                    </h5>
                    <hr>
                  </div>
                </div>
                <div class="row cfd-company-body">
                  <div class="col-xs-12" v-if="order.packages.length >= 1">
                    <div class="collaspe-panel" style="border: 0; padding-left: 0;"><span><i class="fa fa-plus-square"
                                                                                             aria-hidden="true"
                                                                                             data-toggle="collapse"
                                                                                             :data-target="'#' + order._id"></i>Packages</span>
                      <div class="collapse" :id="order._id" aria-expanded="false" style="height: 0px;">
                        <div class="product-text" v-for="item in order.packages"><a :href="'/#/package/' + item.key">
                          <h4>
                            {{item.name}}</h4></a></div>
                      </div>
                    </div>
                  ...
                  <div class="col-xs-12" v-if="order.products.length >= 1">
                    <div class="collaspe-panel" style="border: 0; padding-left: 0;"><span><i class="fa fa-plus-square"
                                                                                             aria-hidden="true"
                                                                                             data-toggle="collapse"
                                                                                             :data-target="'#' + order._id + 'product'"></i>Products</span>
                      <div class="collapse" :id="order._id + 'product'" aria-expanded="false" style="height: 0px;">
                        <div class="product-text" v-for="item in order.products"><a
                          :href="'/#/product/company-formation/' + item.slug"><h4>
                          {{item.title}}</h4></a></div>
                      </div>
                    </div>
                    <hr>
                  ...
                  <div class="col-xs-12" v-if="order.documents.length >= 1">
                    <div class="collaspe-panel" style="border: 0; padding-left: 0;"><span><i class="fa fa-plus-square"
                                                                                             aria-hidden="true"
                                                                                             data-toggle="collapse"
                                                                                             :data-target="'#' + order._id + 'document'"></i>Documents</span>
                      <div class="collapse" :id="order._id + 'document'" aria-expanded="false" style="height: 0px;">
                        <div class="product-text" v-for="item in order.documents"><a :href="'/#/document/' + item.slug">
                          <h4>
                            {{item.title}}</h4></a></div>
                      </div>
                    </div>
